...
int virtio_blk_present(void){return present;}int virtio_blk_read_sector(uint32_t lba,void*buffer){if(!present||!buffer)return-1;request->type=VIRTIO_BLK_T_IN;request->reserved=0;request->sector=lba;*status=0xffu;for(uint32_t i=0;i<512u;i++)data[i]=0;avail->ring[avail->idx%VIRTIO_QUEUE_SIZE]=0;avail->idx++;outw((uint16_t)(io_base+16u),VIRTIO_BLK_QUEUE);for(uint32_t wait=0;wait<VIRTIO_POLL_LIMIT;wait++)if(used->idx!=last_used){last_used=used->idx;if(*status!=0)return-1;for(uint32_t i=0;i<512u;i++)((uint8_t*)buffer)[i]=data[i];return 0;}return-1;}
