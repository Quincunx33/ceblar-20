...
static uint16_t io_base;static int present;
static block_device_t virtio_block={.name="virtio0",.sector_size=512u,.sector_count=0xffffffffULL,.read_sector=virtio_blk_read_sector,.write_sector=virtio_blk_write_sector,.present=0};static dma_buffer_t queue_dma,request_dma;static virtq_desc_t*desc;static volatile virtq_avail_t*avail;static volatile virtq_used_t*used;static virtio_blk_req_t*request;static uint8_t*data;static uint8_t*status;static uint16_t last_used;
...
int virtio_blk_write_sector(uint32_t lba,const void*buffer){if(!present||!buffer)return-1;request->type=1u;request->reserved=0;request->sector=lba;for(uint32_t i=0;i<512u;i++)data[i]=((const uint8_t*)buffer)[i];*status=0xffu;desc[1].flags=VIRTQ_DESC_F_NEXT;avail->ring[avail->idx%VIRTIO_QUEUE_SIZE]=0;avail->idx++;__asm__ volatile("":::"memory");outw((uint16_t)(io_base+16u),VIRTIO_BLK_QUEUE);for(uint32_t wait=0;wait<VIRTIO_POLL_LIMIT;wait++){if(used->idx!=last_used){last_used=used->idx;desc[1].flags=VIRTQ_DESC_F_NEXT|VIRTQ_DESC_F_WRITE;return *status==0?0:-1;}__asm__ volatile("pause");}desc[1].flags=VIRTQ_DESC_F_NEXT|VIRTQ_DESC_F_WRITE;return-1;}
