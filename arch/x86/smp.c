...
void smp_init(void){uint32_t a,b,c,d;cpuid(0,&a,&b,&c,&d);if(a>=1){cpuid(1,&a,&b,&c,&d);apic=(d>>9)&1u;}kprintf("smp: BSP online, CPUID APIC %s; secondary startup pending\n",apic?"available":"unavailable");}
