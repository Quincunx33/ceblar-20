...
static volatile uint32_t*lapic=(volatile uint32_t*)LAPIC_DEFAULT_BASE;static int enabled;
...
void apic_init(void){uint32_t base;if(!smp_apic_available()){serial_write("apic: unavailable, PIC mode retained\n");return;}base=acpi_lapic_base();if(base!=LAPIC_DEFAULT_BASE||read_apic_base()!=base){serial_write("apic: non-default base unsupported, PIC mode retained\n");return;}lapic=(volatile uint32_t*)(uintptr_t)base;write_lapic(LAPIC_SVR,lapic[LAPIC_SVR/4u]|LAPIC_ENABLE);enabled=1;kprintf("apic: local APIC enabled at 0x%x\n",base);}
