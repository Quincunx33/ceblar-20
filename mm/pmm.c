...
static uint8_t page_bitmap[BITMAP_BYTES]; static uint16_t page_refs[MAX_PHYS_PAGES]; static uint32_t total_pages_count, used_pages_count;
...
uint32_t pmm_alloc_page(void){for(uint32_t p=1;p<total_pages_count;p++)if(!bit_test(p)){bit_set(p);page_refs[p]=1;used_pages_count++;return p*PAGE_SIZE;}return 0;}
void pmm_retain_page(uint32_t a){uint32_t p=a/PAGE_SIZE;if(p>0&&p<total_pages_count&&bit_test(p)&&page_refs[p]!=0xffffu)page_refs[p]++;}
void pmm_free_page(uint32_t a){uint32_t p=a/PAGE_SIZE;if(p>0&&p<total_pages_count&&bit_test(p)){if(page_refs[p]>1){page_refs[p]--;return;}page_refs[p]=0;bit_clear(p);if(used_pages_count)used_pages_count--;}}
