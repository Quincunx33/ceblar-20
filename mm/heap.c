...
static uint8_t heap_area[HEAP_BYTES] __attribute__((aligned(16)));static block_t*head;static uint32_t heap_virt_top=HEAP_VIRT_START;
...
void*kmalloc(usize_t n){if(!head)heap_init();if(!n)return 0;n=(n+7u)&~7u;for(;;){for(block_t*b=head;b;b=b->next)if(b->free&&b->size>=n){split(b,n);b->free=0;return(uint8_t*)b+sizeof(block_t);}if(grow()!=0)return 0;}}
void kfree(void*p){if(!p)return;block_t*b=(block_t*)((uint8_t*)p-sizeof(block_t));if(b->magic!=HEAP_MAGIC||b->free)return;b->free=1;merge();}
