...
typedef struct pcb {pid_t pid;uint32_t state;uint32_t ticks;const char*name;cpu_state_t cpu_state;uint8_t kernel_stack[4096];struct pcb*next;uint32_t entry_point;uint32_t user_stack;uint8_t user_mode;struct address_space*address_space;pid_t parent_pid;int32_t exit_code;pid_t wait_for_pid;uint32_t wait_status_ptr;int32_t wait_status;uint32_t pending_signals;uint32_t signal_mask;uint32_t futex_key;uint32_t futex_expected;uint8_t started;struct registers*interrupt_frame;uint32_t initial_frame[15];} pcb_t;
