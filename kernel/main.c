...
void kmain(uint32_t magic, uint32_t mbi_addr) {
    ...
    pit_init(100); pci_init(); dma_init(); virtio_blk_init(); {uint8_t virtio_sector[512];if(virtio_blk_present()){if(virtio_blk_read_sector(0,virtio_sector)==0)serial_write("virtio-blk: sector read ok\n");else serial_write("virtio-blk: sector read unavailable\n");}} keyboard_init(); register_irq_handler(0,(irq_handler_t)timer_irq); register_irq_handler(1,(irq_handler_t)keyboard_irq); pic_unmask(0); pic_unmask(1); ata_init(); fat32_mount(0); storage_smoke(); vfs_init(); net_init(); syscall_init(); run_preemption_test();
