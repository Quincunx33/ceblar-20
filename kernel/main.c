...
void kmain(uint32_t magic, uint32_t mbi_addr) {
    ...
    gdt_init(); tss_init(0); idt_init(); acpi_init(); smp_init(); pmm_init(mbi_addr); vmm_init(); if(vmm_clone_selftest()!=0)panic("address-space clone self-test failed"); serial_write("vmm: clone self-test passed\n"); if(vmm_cow_selftest()!=0)panic("COW self-test failed"); serial_write("vmm: COW self-test passed\n"); heap_init();
