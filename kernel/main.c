...
void kmain(uint32_t magic, uint32_t mbi_addr) {
    ...
    pit_init(100); pci_init(); dma_init(); block_init(); virtio_blk_init(); keyboard_init(); register_irq_handler(0,(irq_handler_t)timer_irq); register_irq_handler(1,(irq_handler_t)keyboard_irq); pic_unmask(0); pic_unmask(1); ata_init(); fat32_mount(0); storage_smoke(); vfs_init(); net_init(); syscall_init(); run_preemption_test();
