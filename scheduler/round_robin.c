...
static void activate(pcb_t*p){p->state=PROCESS_RUNNING;if(p->address_space)vmm_switch_address_space(p->address_space);else vmm_switch_address_space(0);tss_set_kernel_stack((uint32_t)&p->kernel_stack[4096]);current=p;}
